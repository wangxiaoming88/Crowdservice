...
import com.crowdserviceinc.crowdservice.widget.RoundedImageView;
...
public class SendMessageActivity extends ParentActivity implements OnClickListener {
	...
	String from_user_id, to_user_id, job_id, to_user_name;

	Boolean isFromMailbox = false;
	...
	private void init() {
		...
		to_user_name = getIntent().getStringExtra("to_user_name");
		isFromMailbox = getIntent().getBooleanExtra("from_mailbox", false);
		
		txt_customer.setText(to_user_name);
...
//				bm = BitmapFactory.decodeFile(imgPath, btmapOptions);
				...
				bm = Utils.decodeUri(SendMessageActivity.this, selectedImageUri, 162, 182);
			...
			public void run() {
				...
				try {
					Log.d("VIPII", "from_user_id" + from_user_id
							...
							+ " message " + message + " to_user_name " + to_user_name);
