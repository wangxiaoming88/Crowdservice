...
import com.crowdserviceinc.crowdservice.activity.contact.ContactEditDialogActivity;
...
public class JobAndCustomerDetailBaseFragment extends Fragment implements
	...
	private void iniUi(Bundle arguments) {
		...
		if (getActivity() instanceof FindGoferActivity)
			((FindGoferActivity) getActivity()).setTitle("Job and Customer");
		else
	...
	private class ServerCommuniBidjob extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... strParams) {
			...
			JSONObject bidForJob = new JSONObject();

			SharedPreferences loginDB = getActivity().getSharedPreferences(
					LoginActivity.LOGINDATA, 0);

			String userId = Constants.uid;
			String postUserID = job.getUserId();
			String price = jobPrice;
			String categoryId = job.getCategoryId();
			String userName = Constants.username;
			String jobName = job.getName();

			try {
				bidForJob.put("User", userName);
				bidForJob.put("BidUserId", userId);
				bidForJob.put("PostUserId", postUserID);
//				bidForJob.put("JobAmount", price);
				bidForJob.put("JobName", jobName);
//				bidForJob.put("BidDate", date);
				bidForJob.put("JobCategory", categoryId);
				mixpanel.track("JobBidEvent", bidForJob);
