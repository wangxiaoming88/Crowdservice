...
import com.crowdserviceinc.crowdservice.activity.profile.ProfileActivity;
			...
			if (response != null && response.length() > 0) {
				try {
					...
					if (jsonObject.has("status")
						...
						mixpanel = MixpanelAPI.getInstance(SettingsActivity.this, Constants.MIXPANEL_TOKEN);
						JSONObject LogoutEvent = new JSONObject();

						String userName = Constants.username;

						try {
							LogoutEvent.put("User", userName);
							mixpanel.track("LogoutEvent", LogoutEvent);
